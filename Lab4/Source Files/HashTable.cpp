...
int HashTable::GetHashCode(string key)
{
    ...
    {
        hash = _pearsonTable[(hash + (int)key[i]) % _capacity];
    }

    return hash;
}

void HashTable::Rehashig(int capacity)
{
...
HashTableItem* HashTable::AddRightOrder(HashTableItem* item, string key, string value)
{
    if (item == nullptr)
    {
...
HashTableItem* HashTable::ResolveCollision(HashTableItem* item, string key, string value)
{
    if (item == nullptr)
    {
        return new HashTableItem(key, value);
...
HashTableItem* HashTable::Delete(HashTableItem* item, string key)
{
    if (item == nullptr)
    {
        return nullptr;
...
void HashTable::Delete(string key)
{
    ...
    if (_length <= _capacity && _capacity > MinCapacity)
    {
        Rehashig(_capacity / 2);
    }
}

HashTableItem* HashTable::Search(string key, int hashCode)
{
    if (hashCode == -1)
    {
...
HashTableItem** HashTable::GetData()
{
    return _data;
}
